...
def gauss_seidel(A: np.ndarray, b: np.ndarray, x_init: np.ndarray, tol: float = 1e-6, max_iter: int = 50):
    """
    The following code is the implementation of the Gauss-Seidel Method to solve the system of linear equation:
        Ax = b
    ...
    while iter_th < max_iter and err > tol:
        ...
        for n in np.arange(start=1, stop=N+1):
            x[n] = np.dot(G[n-1, :], x)
            print(f'   x[{n}] = {x[n]}')
...
b = np.array([18, 13, -7])
x_init = np.zeros(3)
x_solution, err = gauss_seidel(A, b, x_init, tol=1e-4)
